...
class Game:
    ...
    def __init__(self, starting_tech_positions, colonies,
        ...
        self.achievements = {
            'advancedCityPlanning': False, 'microliteConstruction': False,
            ...
            'heightenedIntelligence': False, 'biomorphicFungi': False
        }
    ...
    def avail_freighters(self):
        return (self.num_freighters
                - 5 * len(self.in_transport)
    ...
    def freighters_needed(self):
        return sum(0 if c.food >= 0 else abs(c.food) for c in self.colonies)

    @property
    def food(self):
        return sum(c.food for c in self.colonies)

    @property
    def pop(self):
        return sum(c.cur_pop for c in self.colonies)

    # if reserve becomes negative, sell buildings until reserve is positive
    # TODO: does the game choose a building at random?
    def sell_buildings(self):
        temp = []
        for c in self.colonies:
            for b in c.buildings:
                if c.buildings[b] and b not in ['terraforming', 'soilEnrichment',
                                                'gaiaTransformation']:
                    temp.append((c, b))

        while self.reserve < 0:
            c, b = random.choice(temp)

            sold_for = Colony.building_data[b].cost // 2
            self.reserve += sold_for
            c.buildings[b] = False

            print(f'{b} was sold at {c.name} for {sold_for} bc')
    ...
    @property
    def rp(self):
        return sum(c.rp for c in self.colonies)

    # money
    @property
    def bc(self):
        return (sum(c.bc for c in self.colonies)
                + floor(.5 * self.food) * (self.food > 0)
                - floor(.5 * self.food_freighters)
                - floor(2.5 * len(self.in_transport))
                )

    # distribute food using freighters
    def distribute_food(self):
        self.food_freighters = 0
        for c in self.colonies:
            c.imported_food = 0

        deficit = sum(c.food for c in self.colonies if c.food < 0)
        surplus = sum(c.food for c in self.colonies if c.food > 0)

        condition = True
        while condition:
            for c in self.colonies:
                if surplus > 0 and deficit < 0 and self.avail_freighters > 0:
                    if c.food + c.imported_food < 0:
                        c.imported_food += 1
                        surplus -= 1
                        deficit += 1
    ...
    def turn(self):
        ...
        self.cum_rp += sum(c.rp for c in self.colonies)
        self.reserve += self.bc

        # distribute food using available freighters if there is a colony with a
        # food deficit and a colony with a food surplus
        if (any(c.food + c.imported_food < 0 for c in self.colonies)
                and any(c.food > 0 for c in self.colonies)):
        ...
        if (self.res_queue is not None
                and self.stored_rp >= int(1.5 * self.res_queue.cost)):
    ...
    def turn_summary(self):
        ...
        for c in self.colonies:
            cost = building_data[c.build_queue].cost
            temp = [c.num_farmers, c.food, c.num_workers, c.prod,
                    c.num_scientists, c.rp, c.build_queue,
                    str(c.stored_prod) + '/' + str(cost), c.bc, c.cur_pop,
                    c.climate, c.max_pop
                    ]
            self.colonies_summary[-1].append(temp)
        ...
        self.game_summary.append(
            [self.food, self.rp, self.cum_rp, self.bc,
             self.reserve, self.pop, research,
             self.food_freighters, self.num_freighters
             ]
        )

    def print_turn_summary(self, starting_turn=0):
        col_headers = ['farmers', 'net food', 'workers', 'prod.', 'scient.', 'rp',
                       'building', 'progress', 'net bc', 'pop', 'climate', 'pop_max']
        ...
        for col_summary, game_summary in zip(colonies_summary, game_summary):
            print('turn:', turn)
            print()
            print(tabulate(col_summary, headers=col_headers, tablefmt='simple'))
            print()
            print(tabulate([game_summary], headers=game_headers, tablefmt='simple'))
            print()
            print()
            turn += 1
